"""A program for triggering output with a pulse if input is below a threshold time or keep it enabled until input is released."""
...
PULSE_TIME_DEFAULT = const(500)  # time below output is triggered with a pulse
...
INPUT_TRIGGER_TIME_MAP = {}


def check_input(input_pin: Pin, output_pin: Pin, pulse_time=PULSE_TIME_DEFAULT) -> None:
    """Check input pin status then enable the output with a pulse or keep enabled until input is released."""
    # button pressed track press time
    is_input_triggered = input_pin.value() == 0
    if INPUT_TRIGGER_TIME_MAP.get(input_pin) is None and is_input_triggered:
        INPUT_TRIGGER_TIME_MAP[input_pin] = ticks_ms()
    if input_pin in INPUT_TRIGGER_TIME_MAP:
        time_diff = abs(ticks_diff(INPUT_TRIGGER_TIME_MAP[input_pin], ticks_ms()))
        # pulse, enable the output and spawn a timer for disabling that
